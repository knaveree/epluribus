...
class ModelInterface:
	...
	def process_csv(data_path : str) -> list:
		'''Takes in filepath to a csv_file and outputs X and Y tuple as 
		list of duples of matrix vectors, the format native to NeuralNetwork.'''
		pass
	
	def unpickle_df(pickle_path : str) -> list:
		'''Takes in filepath to a pickled dataframe and outputs X and Y tuple 
		as list of duples of matrix vectors, the format native to 
		NeuralNetwork.'''
	
	def unpickle_cooked_data(pickle_path : str):
...
def preprocess_data(mock_MNIST):
