...
class Model:
	def _vectorize(self, inputs : (list, np.ndarray, np.matrix)):
		is_ndarray = isinstance(inputs, np.ndarray)
		is_matrix = isinstance(inputs, np.matrix)
		if not is_matrix and is_ndarray:
		...
		if isinstance(inputs, np.matrix):
			...
			if inputs.shape[1] == 1:
				...
				raise Exception('Invalid dimensions for vector input')
		else:
	...
	def train(
		...
		self._iterate(train_list, epochs, learn_rate, display, 'train')
	...
	def test(
		...
		self._iterate(test_list, 1, .0, display, 'test') 
	...
	def _iterate(self, 
		...
		data = datapack.__dict__[train_or_test]

		for epoch in range(epochs):
			self.epoch = epoch
			for step, XY in enumerate(data):
				X, Y = XY
				self.step = step
				step_loss = self._feedforward(X, Y)
				if direction == 'train':
					self._backpropagate()
					self._update()
				elif direction == 'test':
	...
	def predict(self, X, clear=True):
		Y = [0 for i in range(self.output_size)]
		self._feedforward(X, Y, ignore_Y=True)
...
def random_y(output_size):
	...
	return np.array([1 if i == j else 0 for i in range(10)])
...
def X_data(input_size, dataset_size):
	return np.array([random_x(input_size) 
...
def data(X_data, Y_data):
	...
	return [(X_data[i],Y_data[i]) for i in range(X_data.shape[0])]
...
def test_train(grawp, data, X_data, Y_data):
	grawp.train(data, epochs=10, learn_rate=0.5, display=False)
	for X, Y in zip(X_data, Y_data):
		Y_hat = grawp.predict(X) 
		Y = np.matrix([Y]).T
		assert isinstance(Y - Y_hat, np.matrix)

def test_test(grawp, data):
	grawp.test(data, display=False)
